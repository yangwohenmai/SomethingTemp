...
def del_ST(back_dir,factor_else2):
    factor_else2_dir = os.path.join(back_dir, 'test_pre2.csv')
    st_dir = os.path.join(cn.stock_file_dir, 'st1.csv')

    hh = 'F'
    if hh=='True':
        factor_else = pd.read_table(r'E:\new file\lstm\huice_shuju\st.csv', sep=',', index_col=0)  # 读取训练数据集
        n=[]
        for i in factor_else.columns:
            if i[:2] == '60':
                m = 'sh' + i[:6]
            elif i[:2] == '00' or i[:2] == '30':
                m = 'sz' + i[:6]
            n.append(m)
        factor_else.columns=n
        o=0
        for m in factor_else.columns:
            try:

                o+=1
                b=factor_else[m].reset_index()
                b['code']=[m]*len(b)
                b.set_index(['code'],inplace= True)
                df=b.reset_index()
                df.columns = ['code', 'date', 'ST']
                df['ST'] = df['ST'].astype('str')
                df  = df[df['ST'].isin(['True'])]
                ...
                df.round(2).to_csv(st_dir, mode='a', index=False, header=header)
                #data)
            except:
                print (o,m)

    factor_else3 = pd.read_table(st_dir, sep=',')  # 读取训练数据集
    factor_else3['code_date']=factor_else3['code']+factor_else3['date']
    factor_else2['code_date']=factor_else2['code']+factor_else2['date']
    d1=factor_else3['code_date'].tolist()
    d2=factor_else2['code_date'].tolist()
    d3=list(set(d2)-set(d1))
    print(len(d1),len(d2),len(d3))
    factor_else4  = factor_else2[factor_else2['code_date'].isin(d3)]
    del factor_else4['code_date']
    print(len(factor_else4),len(factor_else2))
    factor_else4.to_csv(factor_else2_dir, mode='w', index=False, header=True)
    return factor_else4
...
def read_pre(dataset_dir3):
    mor_Stock_price = pd.read_table(cn.mor_Stock_price, sep=',')
    limit_up = 1+cn.kk_all[0]
    #limit_up = '30'
    mor_Stock_price = mor_Stock_price.iloc[:,[0,1,limit_up]]
    mor_Stock_price.columns = ['daima', 'date', 'limit_up']
    mor_Stock_price = mor_Stock_price[mor_Stock_price['limit_up'] > 9.5]

    data_pre = pd.read_table(dataset_dir3, sep=',', header=None)  # 预测明细
    data_pre.columns = ['code', 'date', 'rise_down', 'class', 'pro', 'pro0', 'pro1', 'pro2']

    mor_Stock_price['hebing'] = mor_Stock_price['daima'] + mor_Stock_price['date']
    data_pre['hebing'] = data_pre['code'] + data_pre['date']
    a1 = set(data_pre['hebing'].tolist())
    a2 = set(mor_Stock_price['hebing'].tolist())
    a3 = list(a1 - a2)
    data_pre = data_pre[data_pre['hebing'].isin(a3)]
    data_pre = data_pre[data_pre['class'] == 2].reset_index(drop=True)
    data_pre = data_pre.loc[:, ['code', 'date', 'rise_down', 'class', 'pro', 'pro0', 'pro1', 'pro2']]
    data_pre.to_csv(r'./file/Testdata.csv', mode='w', index=False, header=True)
    return data_pre

# 10只股票的平均回测
def back_trader(predict_data,back_data,trainFileName255):


    predict_data = read_pre(predict_data) # 读取预测明细，并排除涨停的股票数据
    #print(predict_data)
    predict_data=del_ST(trainFileName255,predict_data)  # 预测明细中，排除ST的股票
    print(2222, len(predict_data))

    #zheng_he(predict_data, trainFileName255)  # 输出辅助因子到明细中农
    ...
    for y in date_list:
        ...
        m29 = sh_data[hh]  # 指数收益率
        n9 = n9 * (1 + m29 / 100)  # 指数涨跌净值
